...
function findByUsername(username) {
  ...
  const result = db.exec(
    'SELECT id, username, password_hash, display_name, email, role, is_active, avatar FROM users WHERE username = ? OR lower(email) = lower(?)',
    [username, username]
  );
...
function findByPlatformAccountId(platformAccountId) {
  const db = getDatabase();
  const result = db.exec(
    `SELECT id, username, display_name, email, role, is_active, tenant_id, platform_account_id, activation_expires_at, activation_used_at
     FROM users
